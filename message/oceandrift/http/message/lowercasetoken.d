...
/++
 ...
 +/
bool equalsCaseInsensitive(hstring a, hstring b) nothrow @nogc
{
...
@safe pure:
    ...
    private this(hstring data) nothrow @nogc
    {
        _data = data;
    }
...
static:
    ...
    ///
    LowerCaseToken makeValidated(string input)
    {
        return LowerCaseToken.makeValidated(hstring(input));
    }

    ///
    LowerCaseToken makeConverted(const(char)[] input)
    {
        import std.ascii : toLower;
        import std.format;
        ...
        hstring hdata = delegate(char[] data) @trusted {
            return assumeImmortal(data);
        }(data);

        return LowerCaseToken(hdata);
    }

    ///
    LowerCaseToken makeSanitized(const(char)[] input) nothrow
    {
        import std.ascii : toLower;
...
bool equalsCaseInsensitive(hbuffer a, const LowerCaseToken b) pure nothrow @nogc
{
